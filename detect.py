...
from Tracker.tracker import Tracker # centroid-feature tracker
...
def run(
    ...
    tracker = Tracker(n_init=5, max_age=100, match_thresh=0.7, iou_thresh=0.5) # initialize tracker
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                wh = wh[:, 2:]
                xy = xy[:, :2]
                outputs = tracker.update(xy, xywhs.cpu(), clss.cpu(), confs.cpu(), imc)
                ...
                if len(outputs) > 0:
                    for _, (output) in enumerate(outputs):
                        ...
                        if save_img or save_crop or view_img:  
                            ...
                            label = None if hide_labels else (names[c] if hide_conf else f'{id}') 
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    if vid_path[i] != save_path:  # new video
                        ...
                        tracker = Tracker(n_init=3, max_age=100, match_thresh=0.7, iou_thresh=0.8)
